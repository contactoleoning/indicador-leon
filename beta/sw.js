...
// Nombre propio de la beta. El almacenamiento de cachés es por dominio, no
// por carpeta: si la beta usara la limpieza de siempre ("borrar todo lo que
// no sea mi caché"), al activarse dejaría sin caché offline a la app real,
// que vive en el mismo dominio. Por eso el nombre lleva prefijo propio y la
// limpieza de más abajo solo toca las cachés de la beta.
var CACHE = "indicador-leon-BETA-v1";
var CACHE_PREFIJO = "indicador-leon-BETA";
  ...
  e.waitUntil(
    caches.keys().then(function (keys) {
      return Promise.all(keys.filter(function (k) {
        return k !== CACHE && k.indexOf(CACHE_PREFIJO) === 0;
